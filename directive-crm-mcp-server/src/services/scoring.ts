...
export function assessStormRisk(options: {
  ...
  const severeAlerts = alerts.filter((a) =>
    STORM_RISK_THRESHOLDS.highAlert.includes(a.event)
  );
  if (severeAlerts.length > 0) {
    score += 40;
    factors.push(
      `${severeAlerts.length} active severe weather alert(s): ${severeAlerts.map((a) => a.event).join(', ')}`
    );
  }
  ...
  if (moderateAlerts.length > 0) {
    score += 15;
    factors.push(
      `${moderateAlerts.length} active moderate weather alert(s): ${moderateAlerts.map((a) => a.event).join(', ')}`
    );
  }
  ...
  if (recentLargeHail.length > 0) {
    score += 30;
    factors.push(
      `${recentLargeHail.length} large hail event(s) in past ${recentHailDays} days (>${STORM_RISK_THRESHOLDS.highHailSize}")`
    );
  }
...
export function formatMarkdown(data: Record<string, unknown>): string {
  const lines: string[] = [];
