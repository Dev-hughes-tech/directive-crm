...
function formatPropertyReportMarkdown(report) {
  ...
  if (report.weather.alerts.length > 0) {
    md += '\n\n## Active Alerts\n';
    report.weather.alerts.forEach((a) => {
      md += `### ${a.event} (${a.severity})\n**Headline:** ${a.headline}\n**Valid:** ${a.onset} to ${a.expires}\n**Description:** ${a.description}\n\n`;
    });
  } else {
    md += '\n\n## Active Alerts\nNone\n';
  }
  ...
  if (report.hailEvents.length > 0) {
    md += '\n\n## Recent Hail Events\n';
    report.hailEvents.forEach((e) => { md += `- ${e.date}: ${e.size}" hail (${e.severity})\n`; });
  } else {
    md += '\n\n## Recent Hail Events\nNone\n';
  }
  return md;
...
server.tool(
  ...
  async ({ lat, lng, address }) => {
    ...
    if (alerts.length === 0) return { content: [{ type: 'text', text: '## Weather Alerts\n\nNo active alerts.' }] };
    let text = '## Active Weather Alerts\n\n';
    alerts.forEach((a) => { text += `### ${a.event} (${a.severity})\n**Headline:** ${a.headline}\n**Valid:** ${a.onset} to ${a.expires}\n**Description:** ${a.description}\n\n`; });
    return { content: [{ type: 'text', text: truncateOutput(text, CHARACTER_LIMIT) }] };
  }
);
...
server.tool(
  ...
  async ({ lat, lng, address, days }) => {
    ...
    if (daytime.length === 0) return { content: [{ type: 'text', text: 'No forecast data available.' }] };
...
server.tool(
  ...
  async ({ lat, lng, address, days, radius_miles, response_format }) => {
    ...
    if (events.length === 0) return { content: [{ type: 'text', text: `No hail events found in past ${days} days.` }] };
